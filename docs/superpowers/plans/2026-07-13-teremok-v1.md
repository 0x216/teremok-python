...
- **Per-method result queues** (final-review Critical fix): `MockedSession._results` is `dict[type[TelegramMethod], deque[Response]]`, not the global FIFO the plan's Task 2 described — queued results are keyed by method class so handler call order cannot misroute them; other calls fall through to the auto-responder. `session.add_result(method_cls, response)`; `MockBot.add_result` public signature unchanged. Quirk #12.
- **Coverage table covers update types** (final-review): `gen_coverage.py` renders a second table from `Update.model_fields` (dedicated builders vs `MockUpdate(...)` escape hatch), per the spec's "every update type" promise.
