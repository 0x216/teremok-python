...
| 12 | `add_result` targets a specific method type | ✅ handled | queued results are keyed by method class; calls to other methods keep auto-responding, so handler call order can't misroute a queued result; test: `test_add_result_targets_specific_method_not_call_order` |
...
| 15 | Text/caption lengths and entity offsets are counted in **UTF-16 code units**, not Python `len()` | ✅ handled | `_utf16_len` in `validation.py`; astral emoji (e.g. 😀) are one Python `str` element but two UTF-16 units - `len()` would check the 4096-char limit and entity offsets against the wrong scale; tests: `test_length_counted_in_utf16_units`, `test_entity_in_range_passes_with_utf16_offsets` |
| 16 | `Bot(default=DefaultBotProperties(parse_mode=...))` bots get validated even when a call omits `parse_mode=` | ✅ handled | `_resolve_parse_mode` resolves aiogram's `Default` sentinel via `bot.default` before validation runs, so default-parse_mode bots aren't silently skipped; test: `test_default_parse_mode_is_validated` |
